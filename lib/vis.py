...
def draw_graph_with_values(G, dot_output_file_name, maximum=None,
                           minimum=None):
    ...
    for v in G.nodes():
        ...
        if G.node[v]["value"] != 0.0:
            output_file.write("\"" + str(v) + "\" [shape=\"circle\",label=\"\",style=filled,fillcolor=\"" +
                              str(color) + "\",penwidth=\"2\",fixedsize=true,width=\"1\",height=\"1\"];\n")
        else:
            output_file.write("\"" + str(v) + "\" [shape=\"circle\",label=\"\",style=filled,fillcolor=\"" + str(
                color) + "\",penwidth=\"0\",fixedsize=true,width=\"1\",height=\"1\"];\n")

    for edge in G.edges():
        output_file.write("\"" + str(edge[0]) + "\" -- \"" + str(edge[1]) +
                          "\"[dir=\"none\",color=\"black\",penwidth=\"1\"];\n")
...
def draw_graph_dynamic_values(G, FT, fig_output_file_name):
    ...
    for i in range(FT.shape[0]):
        ...
        os.system("sfdp -Goverlap=prism -Tsvg dyn_graph-" +
                  str(i) + ".dot > dyn_graph-" + str(i) + ".svg")
        os.system("rm dyn_graph-" + str(i) + ".dot")
        svg_names = svg_names + " dyn_graph-" + str(i) + ".svg"

    os.system("python lib/svg_stack-master/svg_stack.py --direction=v --margin=0 " +
...
def draw_partitions_with_values(G, partitions, dot_output_file_name, maximum=None, minimum=None):
    ...
    for v in G.nodes():
        ...
        if G.node[v]["value"] != 0.0:
            output_file.write("\"" + str(v) + "\" [shape=\"circle\",label=\"\",style=filled,fillcolor=\"" + str(
                color) + "\",penwidth=\"2\",fixedsize=true,width=\"0.5\",height=\"0.5\"];\n")
        else:
            output_file.write("\"" + str(v) + "\" [shape=\"circle\",label=\"\",style=filled,fillcolor=\"" + str(
                color) + "\",penwidth=\"0\",fixedsize=true,width=\"0.5\",height=\"0.5\"];\n")

    for edge in G.edges():
        if part_map[edge[0]] == part_map[edge[1]]:
            output_file.write("\"" + str(edge[0]) + "\" -- \"" + str(edge[1]) +
                              "\"[dir=\"none\",color=\"black\",penwidth=\"4\"];\n")
        else:
            output_file.write("\"" + str(edge[0]) + "\" -- \"" + str(edge[1]) +
                              "\"[dir=\"none\",color=\"black\",penwidth=\"1\"];\n")
...
def draw_graph(G, dot_output_file_name):
    ...
    for v in G.nodes():
        ...
        output_file.write("\"" + str(v) + "\" [shape=\"circle\",label=\"\",style=filled,fillcolor=\"" + str(
            color) + "\",penwidth=\"2\",fixedsize=true,width=\"1\",height=\"1\"];\n")
